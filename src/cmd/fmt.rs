...
impl FormatFeaturesCmd {
	pub fn run(&self, global: &GlobalArgs) {
		...
		if offenders.is_empty() {
			log::info!("Checked {} crate{}: all formatted", meta.packages.len(), plural(meta.packages.len()));
			return;
		}
		...
		for (path, pkg, features) in offenders.iter() {
			...
			let can_modify = path.starts_with(&allowed_dir) || self.modify_paths.iter().any(|p| path.starts_with(p));
			if !can_modify {
				log::warn!("Not allowed to modify {} outside of: {}", path.display(), allowed_dir.display());
				continue;
			}
		...
		log::info!("Checked {} crate{}: {} with unsorted features", meta.packages.len(), plural(meta.packages.len()), offenders.len());

		if self.fix {
			println!("Fixed {} crate{} with unsorted features", global.green(&fixed.to_string()), plural(fixed));

			if fixed != offenders.len() {
				log::error!("ASSERT FAILED THIS IS A BUG: {} crate{} could not be fixed", offenders.len() - fixed, plural(offenders.len() - fixed));
			}
			std::process::exit(0);
		}
